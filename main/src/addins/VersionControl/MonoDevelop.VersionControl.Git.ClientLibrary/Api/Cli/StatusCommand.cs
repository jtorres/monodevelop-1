...
namespace Microsoft.TeamFoundation.GitApi.Cli
{
    internal class StatusCommand : GitCommand
    {
        ...
        private delegate void LineParserDelegate(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count);
        ...
        public IStatusSnapshot ReadSnapshot(StatusOptions options)
        {
            using (var command = new StringBuffer(Command))
            {
                ...
                command.Append(" --porcelain=v2 --branch");
                ...
                switch (options.AheadBehind)
                {
                    ...
                    case StatusAheadBehind.No:
                        command.Append(" --no-ahead-behind");
                        break;

                    case StatusAheadBehind.Yes:
                        command.Append(" --ahead-behind");
                        break;
                }
                ...
                switch (options.UntrackedFiles)
                {
                    ...
                    case StatusUntrackedFiles.None:
                        command.Append(" --untracked-files=none");
                        break;

                    case StatusUntrackedFiles.Normal:
                        command.Append(" --untracked-files=normal");
                        break;
                }
                ...
                switch (options.Ignored)
                {
                    ...
                    case StatusIgnored.No:
                        command.Append(" --ignored=no");
                        break;

                    case StatusIgnored.Traditional:
                        command.Append(" --ignored=traditional");
                        break;
                }
                ...
                switch (options.IgnoreSubmodules)
                {
                    ...
                    case StatusIgnoreSubmodule.Dirty:
                        command.Append(" --ignore-submodules=dirty");
                        break;
                ...
                {
                    // TODO.GitApi: consider quote/escaping path?
                    command.Append(" -- ")
                           .Append(options.Path);
                }
        ...
        private IStatusSnapshot ExecuteStatus(string command)
        {
            ...
            using (IProcess process = CreateProcess(command))
            {
                ...
                try
                {
                    ...
                    while (read < buffer.Length && (r = process.StdOut.Read(buffer, read, buffer.Length - read)) > 0)
                    {
                        ...
                        while (index < read)
                        {
                            ...
                            ParseLine(snapshot, buffer, index, (eol - index));
                ...
                catch (ParseException exception) when (!(exception is StatusParseException))
                {
                    throw new StatusParseException(exception.ParseHint, exception.Buffer, exception.Index);
                }
        ...
        /// </summary>
        internal void ParseLine(StatusSnapshot shapshot, ByteBuffer buffer, int index, int count)
        {
            int start = index;
            int end = index + count;
        ...
        private void ParseLineBranchOid(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
        ...
        private void ParseLineBranchHead(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
        ...
        private void ParseLineBranchUpstream(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
            (snapshot.BranchInfo as StatusBranchInfo).SetUpstreamBranchName(new StringUtf8(buffer, index, count));
        }

        private void ParseLineBranchAheadBehind(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
            int start = index;
            int end = index + count;
        ...
        private void ParseLineOnePathChange(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
            int start = index;
            int end = index + count;
            int gap;
            ...
            start = gap + 1;
            StringUtf8 pathCurrent = PathHelper.ParsePathValue(buffer, start, (end - start));
            ...
            StatusEntry statusEntry = new StatusEntry(stagedChange, unstagedChange, submoduleState, modeHead, modeIndex, modeWorktree, shaHead, shaIndex, pathCurrent);
            (snapshot.TrackedItems as StatusTrackedEntries).Add(statusEntry);
        }

        private void ParseLineTwoPathChange(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
            int start = index;
            int end = index + count;
            int gap;
            ...
            if (buffer[start] == (byte)'R')
            {
                ...
                StringUtf8 pathCurrent = PathHelper.ParsePathValue(buffer, start, (gap - start));
                ...
                start = gap + 1;
                StringUtf8 pathOriginal = PathHelper.ParsePathValue(buffer, start, (end - start));
                ...
                var statusEntry = new StatusRenamedEntry(stagedChange, unstagedChange, submoduleStatus, modeHead, modeIndex, modeWorktree, shaHead, shaIndex, score, pathOriginal, pathCurrent);
                (snapshot.TrackedItems as StatusTrackedEntries).Add(statusEntry);
            }
            else if (buffer[start] == (byte)'C')
            {
                ...
                var statusEntry = new StatusCopiedEntry(stagedChange, unstagedChange, submoduleStatus, modeHead, modeIndex, modeWorktree, shaHead, shaIndex, pathCurrent, pathOriginal, score);
                (snapshot.TrackedItems as StatusTrackedEntries).Add(statusEntry);
            }
            else
            {
        ...
        private void ParseLineUnmerged(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
            int start = index;
            int end = index + count;
            int gap;
            ...
            StatusUnmergedEntry unmergedEntry = new StatusUnmergedEntry(unmergedState, submoduleState, stage1mode, stage2mode, stage3mode, modeWorktree, shaStage1, stage2sha, stage3sha, pathCurrent);
            snapshot.AddUnmergedEntry(unmergedEntry);
        }

        private void ParseLineUntracked(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
            StringUtf8 path = PathHelper.ParsePathValue(buffer, index, count);
        ...
        private void ParseLineIgnored(StatusSnapshot snapshot, ByteBuffer buffer, int index, int count)
        {
            StringUtf8 path = PathHelper.ParsePathValue(buffer, index, count);
        ...
        private static int GetNextTokenLength(
            ...
            string errmsg)
        {
            int gap = buffer.FirstIndexOf(delimiter, start, (index + count - start));
            if (gap < 0)
        ...
        /// <returns></returns>
        internal static TreeDifferenceType ParseTreeDifferenceType(byte col)
        {
            if (col == (byte)'.')
            {
                col = (byte)' ';
            ...
            return DiffCommand.GetDifferenceType((char)col);
        }
        ...
        internal static StatusSubmoduleState ParseSubmoduleStatus(ByteBuffer buffer, int index, int count)
        {
            // Expect "N..." | "S[C.][M.][U.]"
        ...
        /// </summary>
        internal static TreeEntryDetailMode ParseOctalMode(ByteBuffer buffer, int index, int count)
        {
        ...
        /// </summary>
        internal static ObjectId ParseSha(ByteBuffer buffer, int index, int count)
        {
            return ObjectId.FromUtf8(buffer, index);
        }
        ...
        /// </summary>
        internal static int ParsePositiveIntValue(ByteBuffer buffer, int index, int count)
        {
            int value;
        ...
        /// </summary>
        private static bool ParseQuestionMark(ByteBuffer buffer, int index, int count)
        {
            if (buffer == null)
            {
