    ...
    internal class RevListCommand : GitCommand
    {
        ...
        public bool HasCommits()
        {
            ...
            using (var command = new StringBuffer(Command))
            {
                ...
                command.Append(" --all --max-count=1 HEAD");
        ...
        internal static void ApplyOptions(StringBuffer command, HistoryOptions options, string until, string since)
        {
            if (command == null)
            ...
            switch (options.Order)
            {
                ...
                {
                    command.Append(" --author-date-order --reverse");
                }
                break;
                ...
                {
                    command.Append(" --date-order --reverse");
                }
                break;
                ...
                {
                    command.Append(" --topo-order --reverse");
                }
                break;
            ...
            switch (options.Simplification)
            {
                ...
                {
                    command.Append(" --simplify-merges");
                }
                break;
            ...
            {
                command.Append(" --max-count=")
                       .Append(options.MaxCount);
            }

            if ((options.Flags & HistoryOptionFlags.CherryPick) != 0)
            {
                command.Append(" --cherry-pick");
            }
            ...
            if ((options.Flags & HistoryOptionFlags.RightOnly) != 0)
            {
                ...
                command.Append(" --right-only");
            }
            if ((options.Flags & HistoryOptionFlags.UseBitmapIndex) != 0)
            {
                command.Append(" --use-bitmap-index");
            }
            if ((options.Flags & HistoryOptionFlags.WalkReflogs) != 0)
            {
            ...
            {
                command.Append(' ')
                       .Append(options.HintPath);
            }
        }

        internal static void ApplyOptions(StringBuffer command, HistoryOptions options, CommitRange range)
