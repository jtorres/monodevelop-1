...
namespace Microsoft.TeamFoundation.GitApi.Cli
{
    internal class ConfigCommand : GitCommand
    {
        ...
        public IReadOnlyList<ConfigurationEntry> ReadConfiguration()
        {
            var cachedConfigPaths = CacheConfigPaths();

            using (var command = new StringBuffer(Command))
            {
                command.Append(" --show-origin --list -z");
                    ...
                    using (Tracer.TraceCommand(Command, command, userData: _userData))
                    {
                        ...
                        while ((read < buffer.Length) && (r = process.StdOut.Read(buffer, read, buffer.Length - read)) != 0)
                        {
                            ...
                            while (get < read)
                            {
                                ...
                                var configEntry = new ConfigurationEntry(key, value, currentLevel, source);
                ...
                catch (ParseException exception) when (ParseHelper.AddContext($"{nameof(ConfigCommand)}.{nameof(ReadConfiguration)}", exception, command))
                {
        ...
        public void Set(string name, string value, ConfigurationLevel level)
        {
            ...
            using (var command = new StringBuffer(Command))
            {
                ...
                //-// TODO:GitApi: Need better solution for escaping quotes and quoting arguments/paths
                command.Append(" ")
                       .Append(name)
                       .Append(" \"")
        ...
        /// </summary>
        private void ApplyOptions(StringBuffer command, ConfigurationLevel level)
        {
            ...
            switch (level)
            {
                ...
                case ConfigurationLevel.Portable:
                    ...
                    {
                        command.Append(" --file \"")
                               .Append(PathHelper.EscapePath(portableConfigPath))
                               .Append("\"");
                ...
                case ConfigurationLevel.Xdg:
                    ...
                    {
                        command.Append(" --file \"")
                               .Append(PathHelper.EscapePath(xdgConfigPath))
                               .Append("\"");
