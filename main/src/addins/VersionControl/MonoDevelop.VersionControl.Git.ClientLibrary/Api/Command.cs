...
namespace Microsoft.TeamFoundation.GitApi
{
    ...
    public abstract class Command : Base
    {
        ...
        protected IProcess CreateProcess(string command)
            => _startProcessCallback(command);
        ...
        protected int Execute(string command, out StringUtf8 standardError, out StringUtf8 standardOutput)
        {
            ...
            {
                var stderrTask = Task.Run(() => { return Extensions.ReadToEndUtf8(process.StdErr); });
                var stdoutTask = Task.Run(() => { return Extensions.ReadToEndUtf8(process.StdOut); });

                process.WaitForExit();

                if (!Task.WaitAll(new[] { stderrTask, stdoutTask }, TimeSpan.FromSeconds(10)))
                {
                    Tracer.TraceError($"Unable to complete read operation after process has exited.", $"-> command = \"{command}\".", TracerLevel.Diagnostic, userData: _userData);
                    throw new TimeoutException("Unable to complete read operation after process has exited.");
                }
        ...
        internal int ExecuteProgress(string command, IOperation progress)
        {
            if (command == null)
                throw new ArgumentNullException(nameof(command));
            if (progress == null)
        ...
        private IProcess CreateProcessImpl(string command)
        {
            if (command == null)
                throw new ArgumentNullException(nameof(command));
