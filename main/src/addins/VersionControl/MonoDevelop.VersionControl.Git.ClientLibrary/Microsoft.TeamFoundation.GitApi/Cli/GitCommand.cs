...
namespace Microsoft.TeamFoundation.GitApi.Cli
{
    ...
    internal abstract class GitCommand : Command
    {
        ...
        protected void ExecuteCommand(string commandName, string command)
        {
        ...
        internal void TestExitCode(IProcess process, string command, params int[] safeCodes)
            => TestExitCode(process, command, (Task<string>)null, safeCodes);
        ...
        private IProcess StartProcessImpl(string command, bool redirect)
        {
            ...
            #if OSX
            var process = new OsXDetachedProcess (Context)
            #else
