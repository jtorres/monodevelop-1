...
namespace Microsoft.TeamFoundation.GitApi.Internal
{
    internal class PathHelper
    {
        ...
        public bool IsValidRepositoryPath (string path)
        {
            if (path == null || path.Length == 0)
                return false;
        ...
        public string ParsePathValue (string buffer, int index, int count)
        {
            if (buffer == null)
                throw new ArgumentNullException (nameof (buffer));
            if (index < 0)
            ...
            if (buffer[end - 1] != (byte)'"')
                throw new PathParseException ("entry-name", new StringUtf8 (buffer), 0);

            // We assume that the unquoted version of the buffer will be shorter
            // as we unescape special characters.
            var unquoted = new char[buffer.Length];
            int len = 0;
            ...
            while (start < end) {
                ...
                if (b == '"') {
                    ...
                    // already trimmed the end and inline DQUOTEs should
                    // be escaped with a leading backslash.
                    throw new PathParseException ("unexpected d-quote", new StringUtf8 (buffer), 0);
                } else if (b != '\\') {
                    // Normal characters get copied as is.
                    unquoted[len++] = b;
                    continue;
                    ...
                    case '2':
                    case '3': {
                            char c1 = buffer[start++];
                            char c2 = buffer[start++];
                            
                            if (!ParseOctalTriple ((byte)c0, (byte)c1, (byte)c2, out var value))
                                throw new PathParseException ("invalid-octal-sequence", new StringUtf8 (buffer), 0);

                            unquoted[len++] = (char)value;
                        }
                        break;
                    }
                }
            ...
            return new string (unquoted, 0, len);
        }
