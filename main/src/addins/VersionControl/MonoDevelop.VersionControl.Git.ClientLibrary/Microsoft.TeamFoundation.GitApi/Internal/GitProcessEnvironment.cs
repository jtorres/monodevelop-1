...
namespace Microsoft.TeamFoundation.GitApi.Internal
{
    internal class GitProcessEnvironment : Environment
    {
            ...
            : base(environment?.Variables ?? Array.Empty<Variable>(), environment?.WorkingDirectory ?? string.Empty)
        {
            ...
            _stdinPipeName = Win32DetachedProcess.GetStandardPipeName(PipeKind.StandardInput, _processNamespace);
            _stdoutPipeName = Win32DetachedProcess.GetStandardPipeName(PipeKind.StandardOutput, _processNamespace);
            _stderrPipeName = Win32DetachedProcess.GetStandardPipeName(PipeKind.StandardError, _processNamespace);
