...
namespace Microsoft.TeamFoundation.GitApi
{
    ...
    internal class Where : IWhere
    {
        ...
        {
            path = Path.Combine (Path.GetDirectoryName (typeof (Where).Assembly.Location), "GitAskPass");
            if (!File.Exists(path)) {
                path = null;
                return false;
            }
        ...
        public bool FindGitInstallation(string path, KnownDistribution distro, out Installation installation)
        {
            ...
            {
                distro = InstallationDetector.GetDistribution(_context, path);
            }
            
            installation = InstallationDetector.CreateInstallation(_context, path, distro);
        ...
        public bool FindGitInstallations(out List<Installation> installations)
        {
            ...
            if (FindApplication(GitAppName, out findGitPath))
            {
                ...
                if (findGitPath.EndsWith(InstallationDetector.AllVersionGitExe, StringComparison.OrdinalIgnoreCase))
                {
                    string cmdpath = @"cmd\" + InstallationDetector.AllVersionGitExe;
                    ...
                    else if (findGitPath.EndsWith(InstallationDetector.Version2Bin64Path, StringComparison.OrdinalIgnoreCase))
                    {
                        findGitPath = findGitPath.Substring(0, findGitPath.Length - InstallationDetector.Version2Bin64Path.Length);
                    }
                    else if (findGitPath.EndsWith(InstallationDetector.Version2Bin32Path, StringComparison.OrdinalIgnoreCase))
                    {
                        findGitPath = findGitPath.Substring(0, findGitPath.Length - InstallationDetector.Version2Bin32Path.Length);
                    }
                    else if (findGitPath.EndsWith(InstallationDetector.Version1Bin32Path, StringComparison.OrdinalIgnoreCase))
                    {
                        findGitPath = findGitPath.Substring(0, findGitPath.Length - InstallationDetector.Version1Bin32Path.Length);
                    }
                }

                candidates.Add(InstallationDetector.CreateInstallation(_context, findGitPath, KnownDistribution.GitForWindows64v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, findGitPath, KnownDistribution.GitForWindows32v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, findGitPath, KnownDistribution.GitForWindows32v1));
            }

            if (!string.IsNullOrEmpty(reg64HklmPath))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, reg64HklmPath, KnownDistribution.GitForWindows64v2));
            }
            if (!string.IsNullOrEmpty(programFiles64Path))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, programFiles64Path, KnownDistribution.GitForWindows64v2));
            }
            if (!string.IsNullOrEmpty(reg64HkcuPath))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, reg64HkcuPath, KnownDistribution.GitForWindows64v2));
            }
            if (!string.IsNullOrEmpty(reg32HklmPath))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, reg32HklmPath, KnownDistribution.GitForWindows32v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, reg32HklmPath, KnownDistribution.GitForWindows32v1));
            }
            if (!string.IsNullOrEmpty(programFiles32Path))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, programFiles32Path, KnownDistribution.GitForWindows32v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, programFiles32Path, KnownDistribution.GitForWindows32v1));
            }
            if (!string.IsNullOrEmpty(reg32HkcuPath))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, reg32HkcuPath, KnownDistribution.GitForWindows32v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, reg32HkcuPath, KnownDistribution.GitForWindows32v1));
            }
            if (!string.IsNullOrEmpty(programDataPath))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, programDataPath, KnownDistribution.GitForWindows64v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, programDataPath, KnownDistribution.GitForWindows32v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, programDataPath, KnownDistribution.GitForWindows32v1));
            }
            if (!string.IsNullOrEmpty(appDataLocalPath))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, appDataLocalPath, KnownDistribution.GitForWindows64v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, appDataLocalPath, KnownDistribution.GitForWindows32v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, appDataLocalPath, KnownDistribution.GitForWindows32v1));
            }
            if (!string.IsNullOrEmpty(appDataRoamingPath))
            {
                candidates.Add(InstallationDetector.CreateInstallation(_context, appDataRoamingPath, KnownDistribution.GitForWindows64v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, appDataRoamingPath, KnownDistribution.GitForWindows32v2));
                candidates.Add(InstallationDetector.CreateInstallation(_context, appDataRoamingPath, KnownDistribution.GitForWindows32v1));
            }
